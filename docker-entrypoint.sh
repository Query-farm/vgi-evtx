...
#
# Dispatch the single vgi-evtx image into one of its transports:
...
#
# The worker is stateless (each call parses a .evtx from inline BLOB bytes or a
# VARCHAR path), so there is no /data to create and no state env to wire — each
# mode just exec's the binary.
set -e
...
case "${1:-http}" in
  http)
    ...
    exec evtx-worker --http "$@"
    ;;
  tcp)
    shift 2>/dev/null || true
    exec evtx-worker --tcp "0.0.0.0:${PORT_TCP:-8001}" "$@"
    ;;
  stdio)
    shift 2>/dev/null || true
    exec evtx-worker "$@"
