//! `evtx_version()` — return the worker's version string.
...
pub struct EvtxVersion;

impl ScalarFunction for EvtxVersion {
    fn name(&self) -> &str {
        "evtx_version"
    }
    ...
    fn metadata(&self) -> FunctionMetadata {
        FunctionMetadata {
            description: "Returns the evtx worker version string".into(),
            return_type: Some(DataType::Utf8),
            examples: vec![FunctionExample {
                sql: "SELECT evtx.main.evtx_version();".into(),
                description: "Return the evtx worker version string.".into(),
