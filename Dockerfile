...
#
# Single image that serves every network transport of the `evtx` VGI worker:
...
#
# Like vgi-units this worker is STATELESS: it parses a `.evtx` supplied as inline
# BLOB bytes or a VARCHAR path on each call and keeps no per-attach state, so
...
# runtime ships. The workspace MSRV is 1.90; `rust:1-bookworm` tracks a current
# stable toolchain that satisfies it (and the edition-2024 `evtx` 0.12 dep).
...
RUN --mount=type=cache,target=/usr/local/cargo/registry \
    ...
    cargo build --release --locked --bin evtx-worker \
    && cp target/release/evtx-worker /evtx-worker
...
ARG SOURCE_URL=https://github.com/Query-farm/vgi-evtx
...
LABEL org.opencontainers.image.title="vgi-evtx" \
      org.opencontainers.image.description="Windows Event Log (.evtx) parsing for defensive DFIR as a VGI worker for DuckDB/SQL (stdio + HTTP + TCP)" \
...
ENV PORT=8000 \
    ...
    VGI_EVTX_GIT_COMMIT=${GIT_COMMIT}
...
COPY --from=build --chmod=0755 /evtx-worker /usr/local/bin/evtx-worker
