#!/usr/bin/env bash
# Build the browser (`worker:`) build of the evtx VGI worker: evtx-wasm compiled
...
LIB="target/$TARGET/release/libevtx_wasm.a"
...
# which the Arrow encoders overflow; an overflow surfaces as a bare "memory
# access out of bounds" with no stack trace. INITIAL_MEMORY must cover the main
# stack plus every pthread stack up front, or startup stalls growing shared
